...
const RefractorsManager = {
  ...
  async create(refractor) {
    try {
      const [result] = await dbPool.query("INSERT INTO refractors SET ?", refractor);
      return { id: result.insertId, ...refractor};
    } catch (err) {
      console.error(
        "Erreur lors de la création du tube optique :",
        err.message,
      );
      throw err;
    }
  ...
  async update(refractor, id) {
    try {
      const [result] = await dbPool.query("UPDATE refractors SET ? WHERE id = ?", [
        refractor,
        id,
      ]);
      return result;
  ...
  async delete(id) {
    try {
      const [result] = await dbPool.query("DELETE FROM refractors WHERE id = ?", id);
